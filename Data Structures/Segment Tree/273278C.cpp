#include<bits/stdc++.h>

using namespace std;

struct node {
    int v;
};

// MAX SEGMENT TREE
class SegmentTree {
    vector<struct node> tree;
    vector<int> arr;
    struct node nullNode;
    int SIZE;

    struct node single(int v) {
        return {v};
    }
    struct node merge(struct node a, struct node b) {
        return { max(a.v, b.v) };
    }
    void buildTree(int root, int x, int y) {
        if(x == y) {
            if(x < arr.size()) {
                tree[root] = single(arr[x]);
            }
        } else {
            int m = x + y >> 1;
            buildTree((root<<1)|1, x, m);
            buildTree((root<<1)+2, m+1, y);
            tree[root] = merge(tree[(root<<1)|1], tree[(root<<1)+2]);
        }
    }
    void update(int root, int x, int y, int idx, int v) {
        if(x == y) {
            if(x == idx) {
                tree[root] = single(v);
            }
        } else {
            int m = x + y >> 1;

            if(idx > m) {
                update((root<<1)+2, m+1, y, idx, v);
            } else {
                update((root<<1)|1, x, m, idx, v);
            }
            tree[root] = merge(tree[(root<<1)|1], tree[(root<<1)+2]);
        }
    }
    struct node query(int root, int x, int y, int l, int r) {
        if(y < l || r < x) {
            return nullNode;
        }
        if(x >= l && y <= r) {
            return tree[root];
        }
        int m = x + y >> 1;
        return merge(query(root<<1|1, x, m, l, r), query((root<<1)+2, m+1, y, l, r));
    }
    int firstGreater(int root, int x, int y, int v) {
        if(x == y) {
            if(x < arr.size() && tree[root].v >= v) {
                return x;
            }
            return -1;
        }
        if(tree[root].v < v) {
            return -1;
        }
        int m = x + y >> 1;
        int left = firstGreater((root<<1)|1, x, m, v);
        if(left >= 0) {
            return left;
        }
        return firstGreater((root<<1)+2, m + 1, y, v);
    }
public:
    SegmentTree(vector<int> arr) {
        this->arr = arr;
        SIZE = pow(2, 1 + floor(log2(arr.size())));
        nullNode = {INT_MIN};
        tree = vector<struct node> (SIZE*2 - 1, nullNode);
        buildTree(0, 0, SIZE - 1);
    }
    void update(int idx, int v) {
        update(0, 0, SIZE - 1, idx, v);
    }
    int query(int l, int r) {
        query(0, 0, SIZE - 1, l, r);
    }
    int firstGreater(int v) {
        return firstGreater(0, 0, SIZE - 1, v);
    }
    void print() {
        for(auto x: tree) {
            cout << x.v << ' ';
        }
        cout << endl;
    }
};

int main() {
    ios::sync_with_stdio(false);
    cin.tie(NULL);
    int n, m;
    cin >> n >> m;
    vector<int> a(n);
    for(int i = 0; i < n; ++i) {
        cin >> a[i];
    }
    SegmentTree st(a);
    while(m--) {
        int q;
        cin >> q;
        if(q == 1) {
            int i, v;
            cin >> i >> v;
            st.update(i, v);
        } else {
            int x;
            cin >> x;
            cout << st.firstGreater(x) << '\n';
        }
    }
}